...
쉽게 말해 HashSet은 같은 이름표는 하나만 붙일 수 있는 출입 명단라고 생각하면 된다.
...
중복 제거, 존재 여부 확인, 방문 체크해야 할 때 HashSet을 사용하면 코드가 훨씬 이해하기 쉬워진다.

---

## 3. 핵심 아이디어

HashSet은 같은 이름표는 하나만 붙일 수 있는 출입 명단이다.
...
예를 들어 HashSet을 사용할 때는 다음처럼 생각할 수 있다.
...
HashSet은 같은 이름표는 하나만 붙일 수 있는 출입 명단처럼 이해하면 쉽다.
