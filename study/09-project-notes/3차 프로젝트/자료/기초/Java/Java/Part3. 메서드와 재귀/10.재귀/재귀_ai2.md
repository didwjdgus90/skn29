...
쉽게 말해 재귀은 큰 상자를 열면 그 안에 더 작은 같은 모양의 상자가 들어 있는 구조라고 생각하면 된다.
...
팩토리얼, DFS, 트리 탐색처럼 같은 구조가 반복되는 문제 해결해야 할 때 재귀을 사용하면 코드가 훨씬 이해하기 쉬워진다.

---

## 3. 핵심 아이디어

재귀은 큰 상자를 열면 그 안에 더 작은 같은 모양의 상자가 들어 있는 구조이다.
...
예를 들어 재귀을 사용할 때는 다음처럼 생각할 수 있다.
...
재귀은 큰 상자를 열면 그 안에 더 작은 같은 모양의 상자가 들어 있는 구조처럼 이해하면 쉽다.
