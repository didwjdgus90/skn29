...
코드를 기능 단위로 묶어 이름을 붙이고 필요할 때 호출하는 문법이다.

Java에서 메서드은 코드의 구조화, 자료 처리, 성능 개선, 입력 파싱 등 실전 문제 해결에 중요한 역할을 한다.

---

## 2. 어떤 문제를 해결하기 위해 사용하는가?

두 숫자를 더하는 기능을 여러 곳에서 재사용하는 상황에서 메서드을 사용할 수 있다.
...
메서드은 이러한 요구 조건에 맞는 Java의 핵심 도구이다.

---

## 3. 핵심 아이디어

메서드을 사용할 때는 기능보다 먼저 처리 모델을 이해해야 한다.
...
이 코드는 내부적으로 메서드의 규칙에 따라 값을 처리한다.
...
메서드은 Java 문제 풀이에서 자주 등장하는 핵심 주제이다.
