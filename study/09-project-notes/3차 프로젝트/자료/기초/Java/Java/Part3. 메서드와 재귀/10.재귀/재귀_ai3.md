...
메서드가 자기 자신을 다시 호출하여 문제를 더 작은 문제로 나누는 방식이다.

Java에서 재귀은 코드의 구조화, 자료 처리, 성능 개선, 입력 파싱 등 실전 문제 해결에 중요한 역할을 한다.

---

## 2. 어떤 문제를 해결하기 위해 사용하는가?

팩토리얼, DFS, 트리 탐색처럼 같은 구조가 반복되는 문제 해결하는 상황에서 재귀을 사용할 수 있다.
...
재귀은 이러한 요구 조건에 맞는 Java의 핵심 도구이다.

---

## 3. 핵심 아이디어

재귀을 사용할 때는 기능보다 먼저 처리 모델을 이해해야 한다.
...
```text
recursive, base case, call stack
```
...
이 코드는 내부적으로 재귀의 규칙에 따라 값을 처리한다.
...
재귀은 Java 문제 풀이에서 자주 등장하는 핵심 주제이다.
